...
export function renderResult(code,attempt,rows,config){
 ...
 else if(code==='wpt') values=Array.from({length:total},(_,i)=>{const a=rows.find(r=>r.question_number===i+1)?.answer;return Number.isInteger(a?.choice)?a.choice:String(a?.text??'').trim()||null;});
