import {names,escapeHtml as e,scorePapi,scoreDisc,scoreTiu,scoreWpt,mapPersonality} from './scoring.js';
...
export function renderResult(code,attempt,rows,config){
 ...
 else if(code==='wpt') values=Array.from({length:total},(_,i)=>{const a=rows.find(r=>r.question_number===i+1)?.answer;return Array.isArray(a?.choices)?(a.choices.length?a:null):Number.isInteger(a?.choice)?a:String(a?.text??'').trim()||null;});
 ...
 if(issues.length)html+=`<p class="notice">Soal ${issues.join(', ')} tidak cocok dengan versi acuan. Grafik belum dihitung; jawaban asli tersedia di bawah.</p>`;
  ...
  html+=`<div class="papi-wrap">${papiChart(result.chart)}</div>`;
  ...
  const r=scoreDisc(values,config.disc),titles=['GRAPH 1 MOST Mask Public Self','GRAPH 2 LEAST Core Private Self','GRAPH 3 CHANGE Mirror Perceived Self'];
  ...
  html+=['most','least','difference'].map((line,i)=>{const p=r.profiles[line];return `<section class="profile-note"><h4>${titles[i]} · ${e(p?.code||'Profil tidak tersedia dalam tabel')}</h4><p>${e(p?.name||'')}</p><p>${e(p?.description?.filter(x=>typeof x==='string'&&x.trim()).join(' · ')||'')}</p><p><b>Job Match:</b> ${e(p?.job_match||p?.jobMatch||'Belum ada data Job Match pada konfigurasi profil.')}</p></section>`;}).join('');
 }else if(code==='mbti'){
  ...
  if(!r.ready)return html+'<p class="notice">Kunci WPT belum lengkap. Jawaban tetap tersimpan dan ditampilkan.</p>'+table(['Nomor','Jawaban'],r.details.map(x=>[x.number,x.value??'']));
  html+=`<div class="result-stats"><div><strong>${r.correct}/50</strong><span>Jawaban benar</span></div><div><strong>${r.points}</strong><span>Point WPT</span></div><div><strong>${r.blank}</strong><span>Kosong</span></div></div>`;
  html+=r.category?`<p class="wpt-category"><b>${e(r.category.label)}</b></p>`:'<p class="notice">Kategori belum tersedia pada konfigurasi WPT.</p>';
  if(r.answered<50)html+='<p class="notice">Hasil sementara: jawaban belum lengkap.</p>';
  html+=table(['Nomor','Jawaban','Hasil'],r.details.map(x=>[x.number,x.value??'',x.value==null?'Kosong':x.correct?'Benar':'Salah']));
 }else{
