...
export function renderResult(code,attempt,rows,config){
 ...
 const total={test1:90,test2:24,tiu5:30,tiu6:40,mbti:70,wpt:50}[code];
 let values,issues=[];
...
export function printResultReport(candidate,items,config,options={}){
 ...
 win.document.write(`<!doctype html><html lang="id"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Hasil Tes - ${e(candidate?.full_name||'Peserta')}</title><link rel="stylesheet" href="styles.css"><link rel="stylesheet" href="results.css?v=20260923-compact-print"><style>
 @page{size:A4;margin:8mm}*{box-sizing:border-box}body{margin:0;background:#fff;color:#172235;font:10px/1.35 Arial,sans-serif}.report-page{max-width:194mm;margin:auto}.report-head{display:flex;justify-content:space-between;align-items:flex-end;padding-bottom:7px;border-bottom:2px solid #194f86}.report-head h1{margin:0;color:#194f86;font-size:20px}.report-head p{margin:2px 0 0}.report-head small{color:#60758a}.report-meta{display:grid;grid-template-columns:repeat(4,1fr);gap:5px;margin:7px 0 9px}.report-meta div{padding:5px 7px;border:1px solid #d7e2ec;border-radius:5px}.report-meta small{display:block;color:#65758a;font-size:8px}.report-meta strong{display:block;margin-top:1px;font-size:10px}.application-report{margin:8px 0 12px;padding:9px;border:1px solid #cbd9e5;border-radius:7px;break-inside:avoid}.application-report h2{margin:0 0 7px;color:#194f86;font-size:14px}.application-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:4px}.application-grid>div{min-width:0;padding:4px 6px;border:1px solid #dce5ed;border-radius:4px;background:#fbfdff}.application-grid>div.wide{grid-column:1/-1}.application-grid small{display:block;color:#65758a;font-size:7.5px}.application-grid strong{display:block;margin-top:1px;font-size:9px;line-height:1.3;white-space:pre-wrap;overflow-wrap:anywhere}.report-test{margin:0 0 10px;padding-top:3px}.report-test+.report-test{border-top:1px solid #d7e2ec}.report-test>h3{margin:0 0 2px;color:#194f86;font-size:15px}.report-test>p{margin:2px 0 5px}.papi-wrap{max-width:410px!important;margin:3px auto!important}.papi-chart{max-height:390px}.disc-charts{grid-template-columns:repeat(3,1fr)!important;gap:5px!important}.disc-plot{padding:4px!important;border-radius:6px!important;break-inside:avoid}.disc-plot h3{margin:2px;font-size:9px!important}.result-stats{gap:18px!important;margin:7px 0!important}.result-stats strong{font-size:19px!important}.result-stats span{font-size:9px}.score-scale{gap:3px!important;margin:6px 0!important}.score-scale>div{padding:4px 2px!important;font-size:8px!important}.progress{height:6px;margin:5px 0;border-radius:4px;background:#e5edf4;overflow:hidden}.progress span{display:block;height:100%;background:#194f86}.table-wrap{overflow:visible!important;margin:4px 0}.table-wrap table{width:100%;border-collapse:collapse;font-size:8px}.table-wrap th,.table-wrap td{padding:2px 4px!important;border:1px solid #d8e1e9;text-align:left}.table-wrap th{background:#edf4fa;color:#183f61}.profile-note{margin-top:6px!important;padding-top:4px!important;break-inside:avoid}.profile-note h4{margin:0 0 4px!important;font-size:11px}.disc-traits{gap:3px!important}.disc-traits span{padding:2px 4px!important;font-size:8px!important}.disc-job-result{margin-top:7px!important;border-radius:6px!important;break-inside:avoid}.disc-job-heading{padding:6px 8px!important}.disc-job-heading h4{font-size:11px!important}.disc-job-grid article{padding:7px!important}.disc-job-grid h5{margin:0 0 3px!important;font-size:10px!important}.disc-job-grid p{font-size:8.5px!important;line-height:1.35!important}.mbti-summary{display:flex;align-items:center;gap:12px;margin:6px 0}.mbti-type{font-size:25px;font-weight:800;color:#194f86}.mbti-summary p{margin:2px 0}.mbti-bars{display:grid;grid-template-columns:repeat(4,1fr);gap:5px}.mbti-dimension{border:1px solid #d7e2ec;border-radius:5px;padding:4px}.mbti-dimension>div{display:flex;justify-content:space-between}.notice{padding:5px 7px!important;margin:5px 0!important;font-size:9px}details{display:none}.result-answer-table{break-inside:auto}.result-answer-table tr{break-inside:avoid}.no-print{display:block;margin:12px auto 0;padding:8px 14px;border:0;border-radius:5px;background:#194f86;color:#fff;font-weight:700;cursor:pointer}@media print{.no-print{display:none}.report-page{max-width:none}.report-test{break-inside:auto}}
 </style></head><body><main class="report-page"><header class="report-head"><div><h1>PT. GARUDA MAS SEMESTA</h1><p>${options.includeApplication?'FORMULIR APLIKASI & HASIL TES RECRUITMENT':'HASIL TES RECRUITMENT'}</p></div><small>Dokumen HRGA</small></header><div class="report-meta">${identity.map(([label,value])=>`<div><small>${e(label)}</small><strong>${e(value||'-')}</strong></div>`).join('')}</div>${applicationSection}${sections}<button class="no-print" onclick="window.print()">Print / Simpan PDF</button></main></body></html>`);
 win.document.close();
